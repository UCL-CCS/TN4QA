...
def index_to_bitstring(bitstring_len: int, number_particles:int, idx:int) -> np.ndarray:
    """
    ...
    assert np.equal(np.sum(bitstring), n_part), f'number of particles not correct. Expect: {n_part} got: {np.sum(bitstring)}'
    return bitstring
...
def civec_to_state(ci_obj, zero_threhold=None) -> QuantumState:
    ...
    try:
        ...
        ci_amps = ci_obj.to_fcivec(
            cisdvec=ci_obj.ci, norb=norb, nelec=ci_obj.mol.nelec
    ...
    if zero_threhold is None:
        ...
        alpha_ket_int_index, beta_ket_int_index = np.nonzero(np.abs(ci_amps)>zero_threhold)

    nq = 2*norb 
    state_matrix = np.zeros((len(alpha_ket_int_index),
                             nq), dtype=int)
    ...
    for i, (a_idx,b_idx) in enumerate(zip(alpha_ket_int_index, beta_ket_int_index)):
        ...
        sign = (-1) ** np.sum([np.sum(beta_arr[: ind]) for ind in alpha_arr.nonzero()[0]])
