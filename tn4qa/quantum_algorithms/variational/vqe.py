...
class VQE(QuantumAlgorithm):
    def __init__(
        ...
        optimiser: Union[Optimizer, str] = None,
    ) -> None:
    ...
    def set_optimiser(self, optimiser=None):
        ...
        if not optimiser or optimiser == "QNSPSA":
            self._optimiser = qnspsa_optimiser(
                self._ansatz,
                self._max_iterations_vqe,
                opt_dict=self._optimisation_dict,
                index=self._optimisation_index,
            )
        elif optimiser == "ADAM":
            self._optimiser = adam_optimiser(
                self._max_iterations_vqe,
                opt_dict=self._optimisation_dict,
                index=self._optimisation_index,
            )
        elif optimiser == "BFGS":
            self._optimiser = bfgs_optimiser(
                self._max_iterations_vqe,
                opt_dict=self._optimisation_dict,
                index=self._optimisation_index,
            )
        elif optimiser == "COBYLA":
            self._optimiser = cobyla_optimiser(
                self._max_iterations_vqe,
    ...
    def _vqe_driver(self):
        driver = VQE(
            ...
            self._ansatz,
            self._optimiser,
            initial_point=self._initial_points,
        )
        return driver
    ...
    def plot_convergence(self):
        plt.plot(
            self._optimisation_dict["optimisation_number"],
            self._optimisation_dict["optimisation_value"],
        )
        plt.show()
        return
